...
class WidgetLassoRegression(QtWidgets.QWidget):
    ...
    def update_plot(self) -> None:
        ...
        self.canvas_plot.axes.set_title("LASSO Regression")
        ...
        if (self.parent().np_data_x_train.size > 1 and self.parent().np_data_y_train.size > 1
            ...
            lasso_regression_model = linear_model.Lasso(complexity_parameter_value)
            lasso_regression_model.fit(self.parent().np_data_x_train, self.parent().np_data_y_train)

            data_y_pred = lasso_regression_model.predict(self.parent().np_data_x_test)
            ...
            self.label_intercept_value.setText(str(lasso_regression_model.intercept_[0]))
            self.label_coefficient_value.setText(str(lasso_regression_model.coef_[0]))
